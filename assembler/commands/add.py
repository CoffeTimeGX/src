...
class ADD(Command):
    ...
    def decode_bin_instruction(self,instruction:int,registers:list) -> list :
        ...
        alpha_register = self.extract_sub_instruction(instruction,self.position_operand1[self.MSB],self.position_operand1[self.LSB])
        gamma_register = self.extract_sub_instruction(instruction,self.position_result[self.MSB],self.position_result[self.LSB])
        ...
        if not state_bit : 
            beta_register = self.extract_sub_instruction(instruction,self.position_if_register[self.MSB],self.position_if_register[self.LSB])
            operand_2 = registers[beta_register]
        else :
            operand_2 = self.extract_sub_instruction(instruction,self.position_if_number[self.MSB],self.position_if_number[self.LSB])
